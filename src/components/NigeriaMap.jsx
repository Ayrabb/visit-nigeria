...
export default function NigeriaMap() {
  ...
  const getStateFromEvent = (e) => {
    ...
    const codeMap = {
      ab: "abia", ad: "adamawa", ak: "akwaibom", an: "anambra",
      ba: "bauchi", be: "benue", bo: "borno", by: "bayelsa",
      cr: "crossriver", de: "delta", eb: "ebonyi", ed: "edo",
      ek: "ekiti", en: "enugu", fc: "fct", go: "gombe",
      im: "imo", ji: "jigawa", kd: "kaduna", ke: "kebbi",
      kn: "kano", ko: "kogi", kt: "katsina", kw: "kwara",
  ...
  const closeDrawer = () => {
    const svgEl = document.querySelector(".nigeria-map");
    if (svgEl) {
      svgEl.querySelectorAll("path").forEach((p) => p.classList.remove("active-path"));
    }
    setActiveState(null);
  };
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <ul style={{ fontSize: 14, paddingLeft: 18 }}>
                  {activeState.attractions.map((spot) => (
                    <li key={spot} style={{ marginBottom: 4 }}>{spot}</li>
                  ))}
                </ul>
              </div>
            )}
